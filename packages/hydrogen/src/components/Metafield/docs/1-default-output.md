...
| `product_reference`      | A `span` containing the product reference GID when the `reference` field is undefined, or a `ProductProvider` component using the `reference` data.                                        |
