...
| widths?         | <code>(number &#124; string)[]</code>                              | An array of pixel widths to overwrite the default-generated `srcset`. For example, `[300, 600, 800]`. This prop only applies to images from Shopify CDN. 
| decoding?         | <code>('async' &#124; 'sync' &#124; 'auto')</code>                              | The decoding property of the HTMLImageElement interface represents a hint given to the browser on how it should decode the image. Defaults to `async` .    
