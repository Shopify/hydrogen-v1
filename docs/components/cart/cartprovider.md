...
The fragment must use the name `CartFragment` because it's referenced that way in each of the queries and mutations. If you provide a custom `cartFragment`, then it must be a raw string. For GraphQL syntax highlight, use the `gql` utility exported from `@shopify/hydrogen` instead of [`graphql-tag`](https://github.com/apollographql/graphql-tag), since the latter returns an `ASTNode` instead of a `string`.
